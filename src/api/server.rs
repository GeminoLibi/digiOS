...
pub async fn handle_capabilities(State(state): State<AppState>) -> Json<Value> {
    Json(serde_json::json!({
...
pub async fn handle_screenshot(State(state): State<AppState>) -> Result<Json<Value>, StatusCode> {
